data <- read.table("(CS)CU(s)loading8col.txt", header = TRUE)

max_y <- max(data)
plot_colors <- c("black", "blue", "red4")
png(filename="(CS)CU(s)loading8col.png", height = 1000, width = 1450, bg = "white")
...
title(main="CPU Usage (s)_Loading_8Columns (Case Study)", col.main="red", font.main=6)
