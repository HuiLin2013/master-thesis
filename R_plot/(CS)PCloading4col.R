data <- read.table("(CS)PCloading4col.txt", header = TRUE)

max_y <- max(data)
plot_colors <- c("black", "blue", "red4")
png(filename="(CS)PCloading4col.png", height = 1000, width = 1450, bg = "white")
...
title(main="Power Consumption (mw)_Loading_4Columns (Case Study)", col.main="red", font.main=6)
