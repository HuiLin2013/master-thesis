data <- read.table("(CS)MUloading10col.txt", header = TRUE)

max_y <- max(data)
plot_colors <- c("black", "blue", "red4")
png(filename="(CS)MUloading10col.png", height = 1000, width = 1450, bg = "white")
...
axis(2, las=1, at=2*0:max_y, cex.axis = 1)
...
title(main="Memory Usage_Loading_10Columns (Case Study)", col.main="red", font.main=6)
