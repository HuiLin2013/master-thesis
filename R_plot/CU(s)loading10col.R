data <- read.table("CU(s)loading10col.txt", header = TRUE)

max_y <- max(data)
plot_colors <- c("black", "blue", "red4")
png(filename="CU(s)loading10col.png", height = 1000, width = 1450, bg = "white")
...
axis(2, las=1, at=4.0*0:max_y, cex.axis = 1)
...
title(main="CPU Usage (s)_Loading_10Columns", col.main="red", font.main=6)
