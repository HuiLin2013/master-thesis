data <- read.table("(CS)ETstr4col.txt", header = TRUE)

max_y <- max(data)
plot_colors <- c("black", "blue", "red4")
png(filename="(CS)ETstr4Col.png", height = 1000, width = 1300, bg = "white")
...
axis(2, las=1, at=40*0:max_y, cex.axis = 1)
...
title(main="Execution Time_Strings_4Columns (Case Study)", col.main="red", font.main=6)
