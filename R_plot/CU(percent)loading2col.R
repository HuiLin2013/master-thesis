data <- read.table("CU(percent)loading2col.txt", header = TRUE)

max_y <- max(data)
plot_colors <- c("black", "blue", "red4")
png(filename="CU(percent)loading2col.png", height = 1000, width = 1450, bg = "white")
...
axis(2, las=1, at=2.0*0:max_y, cex.axis = 1)
...
title(main="CPU Usage (%)_Loading_2Columns", col.main="red", font.main=6)
title(xlab= "Number of rows", col.lab="red")
title(ylab= "CPU Usage (%) for Loading", col.lab="red")
legend('bottomright', max_y, names(data), cex=1.5, col=c("black", "black","blue", "blue","red4","red4"), pch=c(15,15,16,16,17,17), lty=c(1,2,1,2,1,2));
