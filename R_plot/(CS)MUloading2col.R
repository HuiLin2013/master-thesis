data <- read.table("(CS)MUloading2col.txt", header = TRUE)

max_y <- max(data)
plot_colors <- c("black", "blue", "red4")
png(filename="(CS)MUloading2col.png", height = 1000, width = 1450, bg = "white")
...
title(main="Memory Usage_Loading_2Columns (Case Study)", col.main="red", font.main=6)
