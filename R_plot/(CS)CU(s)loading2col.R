data <- read.table("(CS)CU(s)loading2col.txt", header = TRUE)

max_y <- max(data)
plot_colors <- c("black", "blue", "red4")
png(filename="(CS)CU(s)loading2col.png", height = 1000, width = 1450, bg = "white")
...
axis(2, las=1, at=1*0:max_y, cex.axis = 1)
...
title(main="CPU Usage (s)_Loading_2Columns (Case Study)", col.main="red", font.main=6)
