data <- read.table("(CS)CU(s)loading10col.txt", header = TRUE)

max_y <- max(data)
plot_colors <- c("black", "blue", "red4")
png(filename="(CS)CU(s)loading10col.png", height = 1000, width = 1450, bg = "white")
...
axis(2, las=1, at=1*0:(max_y+1), cex.axis = 1)
...
title(main="CPU Usage (s)_Loading_10Columns (Case Study)", col.main="red", font.main=6)
title(xlab= "Number of rows", col.lab="red")
title(ylab= "CPU Usage (s) for Loading", col.lab="red")
